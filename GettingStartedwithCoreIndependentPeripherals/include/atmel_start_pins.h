 ...
 * \param[in] pull_mode Pin pull mode
 */
static inline void Button0_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTB_set_pin_pull_mode(5, pull_mode);
}

/**
 * \brief Set Button0 data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void Button0_set_dir(const enum port_dir dir)
{
	PORTB_set_pin_dir(5, dir);
}

/**
 * \brief Set Button0 input/sense configuration
 *
 * Enable/disable Button0 digital input buffer and pin change interrupt,
 * select pin interrupt edge/level sensing mode
 *
 * \param[in] isc PORT_ISC_INTDISABLE_gc    = Iterrupt disabled but input buffer enabled
 *                PORT_ISC_BOTHEDGES_gc     = Sense Both Edges
 ...
 *                PORT_ISC_LEVEL_gc         = Sense low Level
 */
static inline void Button0_set_isc(const PORT_ISC_t isc)
{
	PORTB_pin_set_isc(5, isc);
}

/**
 * \brief Set Button0 inverted mode
 *
 * Enable or disable inverted I/O on a pin
 *
 * \param[in] inverted true  = I/O on Button0 is inverted
 *                     false = I/O on Button0 is not inverted
 */
static inline void Button0_set_inverted(const bool inverted)
{
	PORTB_pin_set_inverted(5, inverted);
}

/**
 * \brief Set Button0 level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void Button0_set_level(const bool level)
{
	PORTB_set_pin_level(5, level);
}

/**
 * \brief Toggle output level on Button0
 *
 * Toggle the pin level
 */
static inline void Button0_toggle_level()
{
	PORTB_toggle_pin_level(5);
}

/**
 * \brief Get level on Button0
 *
 * Reads the level on a pin
 */
static inline bool Button0_get_level()
{
